...
| ta-veto-liveness | — | **PUSHED to origin, no PR yet** — 6 commits off `main@f5b7577c` (`785b5350` state, `77be65ca` gate, `b3b7f762` dev-guide; round-1 follow-ups: `2b0c715c` F-B1 static QM `Live:true`+test, `5fd0a958` F-T1a discriminating keying test, `7e931ccf` F-T1b/Demand/NTH/Conc dev-guide prose + 2 comments). All gates green (incl. `-race`). All six round-1 follow-ups landed and verified per-item; no new findings. **T1b closed** (reason-based `no-data`→non-live ruled correct + load-bearing). One accepted residual (F-B1 test builds its own `Live:true` result → won't guard a future deletion of that line). Pushed `f5b7577c..7e931ccf`. Review: [`planning/ta-veto-liveness-review.md`](../planning/ta-veto-liveness-review.md) (Status: DRAFT header until Dean finalizes). Plan: [`planning/ta-veto-liveness-plan.md`](../planning/ta-veto-liveness-plan.md). PR open pending Dean's direction. | `7e931ccf` |
| ta-model-level-demand | — | **PUSHED to origin, no PR yet** — 4 commits (`652307bd` Commit 1, `6f161a5a` Commit 2, `e800ff87` F1 nKV-weighted `avgOL` + regression test, `4a816dde` F2 dev-guide `AnalyzerInput` `ArrivalRate` row); both blocking findings landed, re-reviewed FINAL, no outstanding findings: [`planning/ta-model-level-demand-review.md`](../planning/ta-model-level-demand-review.md). Pushed `f5b7577c..4a816dde`. PR open pending Dean's direction. Plan: [`planning/ta-model-level-demand-plan.md`](../planning/ta-model-level-demand-plan.md). | `4a816dde` |
...
- **TA 0.9 coding — all four review-clean; awaiting Dean's push/PR decision.** Four Type 3 plans
  off `main@f5b7577c`, all internally reviewed: `ta-devguide-fixes` (A, **pushed to origin
  `f931a4e9`, no PR**), `ta-registration-safety` (A′, **pushed to origin `7374be55`, no PR**),
  `ta-model-level-demand` (C, F1+F2 landed → **pushed to origin `4a816dde`, no PR**),
  `ta-veto-liveness` (D, round-2 **APPROVE** → **pushed to origin `7e931ccf`, no PR**). **Remaining
  action = Dean's decision:** whether/when to open PRs for A/A′/C/D. No
  coder work outstanding on any of the four; all four pushed to origin, no PRs. (D detail: coder's per-tuple
...
- **TA forward plan — immediate P0 items (now):**
  - ~~**I-21/I-22/I-23**~~ — **DONE** on `ta-devguide-fixes` (PR A), **pushed to origin `f931a4e9`
    (no PR yet)**. Stale PromQL examples, removed-file reference, and `nKV`/booting-replica note
