...
- **2026-08-07 — Anchor-refactor mission (goldens PR #1513 OPEN; PR-1 `ta-anchor-refactor-v2` = **PR #1516 OPEN**,
  ...
  **C6c has zero edits** — scoped read-only, then held on Dean's call (*"bigger change … send a handoff
  to planner"*); its six questions were **all answered inside the Type-3 refresh** (Q1 extraction
  guidance §2d.5 — capture the value, don't re-derive the rule; Q2 → site (v), converted-not-deleted;
  Q3 → unit-table row 8; Q4 → `W2`, deferred; Q5 → site (iii) moved to C6d; Q6 → T1.4 splits §2d.6), so
  ...
  `ceil(target)` proposal is **ruled out** by the plan's threaded per-role `prcRef` requirement (C2's
  `refreshAnchorSizing` rewrites `vc.PerReplicaCapacity` in place on the picker's backing array every
  iteration). Do not implement C6c from it.
  ...
  **`PRC = 1` sentinel** in `PerReplicaCapacity` (separate eligibility predicate rejected: six gate
  ...
  (`ta-anchor-dynamic-refresh__kickoff` + `review__ta-anchor-dynamic-refresh-checklist` remain `.HOLD`);
  the coder's **C10-first offer is declined**. **Bug #5's currency is GPU space, not replica space** —
