...
| (upstream) priority-weighted rescale | #1452 | **Reviewed 2026-07-27** — `COMMENTED` (not approved); 4 non-blocking-for-Alpha questions (RC-1..RC-4); approval held pending author response to RC-2/RC-4. Review FINAL: [`planning/PR1452-review.md`](../planning/PR1452-review.md). CI green. | (fork branch) |
| ta-devguide-fixes | [#1478](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1478) | **OPEN** 2026-07-27 (base upstream `main`; reviewer+assignee ev-shindin) — 4 docs-only commits, rebased onto `55e24be9`; NTH-1 `port` label fix folded in; internal review FINAL/APPROVE: [`planning/ta-devguide-fixes-review.md`](../planning/ta-devguide-fixes-review.md). Plan: [`planning/ta-devguide-fixes-plan.md`](../planning/ta-devguide-fixes-plan.md). **CI `lint-and-test` red until upstream #1477 merges** (main tip `aa86a2a9` doesn't compile — interfaces→domain fallout; branch based on last-good `55e24be9`); e2e jobs unaffected (head-SHA). | `93742a52` |
| ta-registration-safety | [#1479](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1479) | **OPEN** 2026-07-27 (base upstream `main`; ev-shindin) — 5 commits (`effectiveEnabled` opt-in + startup non-registration log + dev-guide + F1/F2 follow-ups), rebased onto `55e24be9`; review FINAL, no outstanding findings: [`planning/ta-registration-safety-review.md`](../planning/ta-registration-safety-review.md). F3 (cross-package veto coverage) accepted as documented — closer fit for PR D. CI: same as #1478 (red until #1477). | `89337622` |
| ta-veto-liveness | [#1481](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1481) | **OPEN** 2026-07-27 (base upstream `main`; ev-shindin) — 6 commits, rebased onto `55e24be9`; all gates green (incl. `-race`). All six round-1 follow-ups landed + verified; **T1b closed** (reason-based `no-data`→non-live ruled correct + load-bearing). One accepted residual (F-B1 test builds its own `Live:true` result → won't guard a future deletion of that line). Review: [`planning/ta-veto-liveness-review.md`](../planning/ta-veto-liveness-review.md) (Status: DRAFT header until Dean finalizes). Plan: [`planning/ta-veto-liveness-plan.md`](../planning/ta-veto-liveness-plan.md). CI: same as #1478 (red until #1477). | `faab066a` |
| ta-model-level-demand | [#1480](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1480) | **OPEN** 2026-07-27 (base upstream `main`; ev-shindin) — 4 commits (model-level arrival demand + nKV-weighted `avgOL` + `ArrivalRate` dev-guide row), rebased onto `55e24be9`; both blocking findings landed, re-reviewed FINAL, no outstanding findings: [`planning/ta-model-level-demand-review.md`](../planning/ta-model-level-demand-review.md). Plan: [`planning/ta-model-level-demand-plan.md`](../planning/ta-model-level-demand-plan.md). CI: same as #1478 (red until #1477). | `68681542` |
...
- **TA 0.9 coding — all four PRs OPEN upstream; forward-rebase pending #1477.** Four Type 3 plans,
  all internally reviewed, re-rebased onto last-good `55e24be9` and force-pushed 2026-07-27, PRs
  opened to upstream `main` (reviewer+assignee ev-shindin): A [#1478](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1478)
  (`93742a52`), A′ [#1479](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1479)
  (`89337622`), C [#1480](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1480)
  (`68681542`), D [#1481](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1481)
  (`faab066a`). **Two open follow-ups:** (1) CI `lint-and-test` is red on all four until upstream
  #1477 merges (main tip `aa86a2a9` doesn't compile); when #1477 lands, forward-rebase all four
  onto fixed main (same `--onto` pattern) to clear the red. (2) Watch for ev-shindin review
  feedback. No coder work outstanding otherwise. (D detail: per-tuple
  `(namespace, modelID, analyzerName)` keying confirmed correct by review; the reason-based
  `no-data`→non-live liveness mechanism ruled correct + load-bearing; one accepted test residual.)
