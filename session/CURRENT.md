...
- **2026-08-07 — Anchor-refactor mission (goldens PR #1513 OPEN; PR-1 `ta-anchor-refactor-v2` = **PR #1516 OPEN**,
  ...
  **rank** that is never spent (invariant 11, *"priority orders, never scales"*) — *dividing `priority`
  ...
  `ceil(target)` proposal is **superseded twice over**: first by the threaded per-role `prcRef`
  requirement, and then by the GPU-space pivot that **deletes the entire `prcRef` construction** rather
  than refining it (§2d.5 *What stops existing* — *"delete on sight; do not port forward"*; so a
  `prcRef`-based rationale is itself stale wherever it still appears). Do not implement C6c from it.
