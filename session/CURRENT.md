...
- **multi-analyzer-optimizer** — pushed to origin at `1648f3f6` (Phase 1+2+3+cleanup; gates verified). **Now blocked on the rebase onto main**: #1228 merged to `main@d9e4ae1f`, so the branch (based on old threshold tip `b8b823b0`) must cross-rebase onto main before the PR. Coder triggered with the single-pass spec; after it verifies, Dean force-with-lease pushes (rebase rewrites the 16 commits) and opens the PR targeting `main`. See [`planning/multi-analyzer-optimizer-plan.md`](../planning/multi-analyzer-optimizer-plan.md) § "Rebase onto main (post-#1228 merge)". Backup `backup/multi-analyzer-optimizer-pre-rebase@ae456aa0` retained until PR opens.
...
- **Optimizer rebase onto main (coder, in progress).** #1228 merged to `main@d9e4ae1f`, so the optimizer (based on the old threshold tip `b8b823b0`) must rebase onto main before the PR. Instruction is a complete single-pass spec in [`planning/multi-analyzer-optimizer-plan.md`](../planning/multi-analyzer-optimizer-plan.md) § "Rebase onto main (post-#1228 merge)"; coder triggered (`optimizer__rebase-onto-main.md`). After rebase verifies: Dean force-with-lease push (rebase rewrites the 16 commits), then **open PR targeting `main`**. Then close `engine-queue-fix` + drop the backup ref.
