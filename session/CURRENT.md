...
- **TA 0.9 coding — A + A′ + C + D all MERGED; E reviewed-awaiting-push; F awaiting review.**
  ...
  **D [#1481](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1481)** (2026-07-30,
  `f5261c8e`) and **C [#1480](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1480)**
  (2026-07-30, merge `f9f04d81` — the upstream `main` tip) are all **MERGED**. The C/D pair is
  complete. **Remaining follow-up:**
  (1) Trackers **#1495 (C) + #1496 (D) ready to close** (both merged) — pending Dean's confirm. D's 3
  non-blocking review follow-ups ride **PR F Commit 5 (DR-1/DR-2/DR-3)**, not a separate PR.
  (2) **PR F now needs its rebase onto `main@f9f04d81`** — F was based off `main@f5261c8e` (D's merge);
  C has since landed. F's plan flagged this as one ordinary rebase (small conflict surface — C never
  touches `computeLocalDemand`/`resolveITLModel` bodies). Coder task once F's Commit 5 + internal
  review are done.
  (3) Reviewer replies to ev-shindin's round-1 reviews are **POSTED 2026-07-29** (C
  ...
  (4) **`ta-testing` integration branch is stale/moot** — its `db530eed` cut merged the pre-merge C+D
  branches; now both are on `main` (`f9f04d81`), so a plain `main` checkout already contains C+D. A
  combined pre-merge test image is no longer needed unless Dean wants an image cut from a specific
  `main` SHA. Local worktree `ta-model-level-demand` is stale at `b2acffd6` (safe to remove — its
  work is merged). Raise removal with Dean.
