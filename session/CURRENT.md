...
- **2026-08-03 — Anchor-refactor mission (goldens PR #1513 OPEN; PR-1 plan + local worktree; PR-2 stub).**
  ...
  worktree off the goldens tip `a2f49ccf` (interim base; rebases onto `main` after #1513). The two PR-1
  questions the coder raised are now **resolved** (Dean, 2026-08-03; plan commit `500f08d8`): sat-v2
  enablement is **opt-in** — a customized non-empty `analyzers:` list yields exactly what it names (`[TA]`
  alone ⇒ TA-only; no `ApplyDefaults` change) — and the ballot-build **reuses the precomputed saturation
  result** rather than re-invoking `Analyze()` (the load-bearing sat name-skip is kept as a reuse guard,
  not deleted). **No code written yet** — branch still at `a2f49ccf`, not pushed. **PR-2 dynamic-refresh** `ta-anchor-dynamic-refresh`
  = PLAN STUB (deferred until PR-1 lands). Open before coding: **Mechanism A vs B** (§0, before Commit 2) +
...
- **anchor-refactor mission (ta-anchor-goldens #1513 → ta-anchor-refactor PR-1 → ta-anchor-dynamic-refresh PR-2):**
  ...
  sat name-skip stays as a reuse guard, not deleted). Still open before coding: the **Mechanism A/B**
  decision (§0). PR-2 is a deferred stub (do NOT start until PR-1 lands). Carry the `withSatEntry`-stability
  check (goldens review Finding 2) into PR-1. Plans: [`planning/ta-anchor-refactor-plan.md`](../planning/ta-anchor-refactor-plan.md),
  [`planning/ta-anchor-dynamic-refresh-plan.md`](../planning/ta-anchor-dynamic-refresh-plan.md).
