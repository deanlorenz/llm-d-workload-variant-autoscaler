...
- **TA 0.9 coding — FULLY LANDED (all six PRs MERGED 2026-07-30).**
  **A [#1478](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1478)** + **A′
  [#1479](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1479)** (2026-07-28),
  ...
  **E [#1502](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1502)** (merge `1d5553ee`),
  **F [#1503](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1503)** (merge `6bfb73e1`,
  tip `7bf13b33`). **`main` tip = `6bfb73e1`.** All trackers **#1495 / #1496 / #1497 CLOSED**; C and F
  have no dedicated tracker (under the epics). Reviewer replies to ev-shindin's round-1 C/D reviews
  POSTED 2026-07-29. **Remaining follow-up (all optional / GitHub-write, need Dean's direction):**
  (1) **Epics #1492/#1493/#1494 + adopted #1005** — all six PRs merged; check whether they should be
  updated/closed (not done — GitHub-write).
  (2) **3 optional test gaps** ev-shindin declined to add on F #1503 (Tier-2 `validITLModel` rejection;
  `validITLModel` w/ `Inf B`; `computeLocalDemand` non-positive-cap/negative-ITL skips) — **Type 3 plan
  now written** ([`planning/ta-itl-demand-test-gaps-plan.md`](../planning/ta-itl-demand-test-gaps-plan.md));
  deferred, no branch/worktree/owner (Dean: "leave as is, just document"). Cut a branch only when a coder
  is assigned.
  (3) **PR #1501 ask #1 watch** — if the author merges without gating `RegisterRateCapacityQueries` on
  `EnableRateAnchoredK2`, the default-config per-cycle Prometheus-query-load regression ships (load-only,
  not correctness). See PR Status row.
  (4) **Governance open Q (retrospective, F merged):** was the F coder's 2nd unprompted §4a
  commit-message history-rework (`reset --soft`+recommit, tree byte-identical) an acceptable extension
  of the earlier approval? Captured in the § Next steps governance item (2026-07-29 §4a bullet).
  (5) **`ta-testing` integration branch + local `ta-model-level-demand` worktree are stale/moot** —
  all six PRs on `main` (`6bfb73e1`) supersede the pre-merge cut. See the refresh-trigger item below +
  the `ta-testing` PR Status row; raise worktree/branch removal with Dean.
