...
- **2026-08-03 — Anchor-refactor mission (goldens PR #1513 OPEN; PR-1 plan + local worktree; PR-2 stub).**
  ...
  kickoff; **land-first** decided). **PR-1 static core** `ta-anchor-refactor` = **PLAN REVIEWED &
  FOLDED-IN** (2026-08-04, commits `68bda1a1`/`192ae06b`) + **LOCAL** worktree off the goldens tip
  `a2f49ccf` (interim base; rebases onto `main` after #1513). The two PR-1 questions the coder raised are
  **resolved** (Dean, 2026-08-03; plan commit `500f08d8`): sat-v2 enablement is **opt-in** — a customized
  non-empty `analyzers:` list yields exactly what it names (`[TA]` alone ⇒ TA-only; no `ApplyDefaults`
  change) — and the ballot-build **reuses the precomputed saturation result** rather than re-invoking
  `Analyze()` (the load-bearing sat name-skip is kept as a reuse guard, not deleted). A subsequent
  coding-plan review (`planning/ta-anchor-refactor-review.md`, still DRAFT) found 2 MAJOR correctness
  findings + 5 coder-executability blockers + minor polish; **all folded into the plan** — notably the
  **queueing-model path is now disabled via an explicit-error dispatch refusal** (never a silent fallback
  to sat-v2; classified DEFERRED §12, not deprecated) and the **Mechanism A/B copy-mechanism choice is
  resolved** (§0: variant (ii) — always full-copy sat-v2's (a)+(b) into the anchor, overwrite (b) from
  `ballot[0]` when non-empty). Plan doc now internally consistent and coder-executable. **No code written
  yet** — branch still at `a2f49ccf`, not pushed. **PR-2 dynamic-refresh** `ta-anchor-dynamic-refresh` =
  PLAN STUB (deferred until PR-1 lands). Still open: mark the review FINAL (review agent/Dean's call) +
  design-doc §anchor "replace"-wording annotation (§12). Design authority
...
| ta-anchor-refactor | — | **PLAN REVIEW FOLD-IN COMPLETE (2026-08-04) + LOCAL worktree — not started.** PR-1 static core of the anchor refactor (engine builds `anchor = copy(sat-v2)` + passes enabled-analyzer ballot; zero combine-arithmetic change; decision-set-identity ship gate via #1513 goldens; opt-in enablement). Worktree cut off goldens tip `a2f49ccf` (interim base; rebases onto `main` after #1513); **no code written — still at `a2f49ccf`**. The coder's 2 PR-1 questions are **resolved** (Dean, 2026-08-03; plan commit `500f08d8`): (1) sat-v2 enablement is **opt-in** — a customized non-empty `analyzers:` list is taken literally (`[TA]` ⇒ TA-only), no `ApplyDefaults` change; (2) the ballot-build **reuses the precomputed saturation result** (load-bearing name-skip kept as a reuse guard, not deleted). A coding-plan review (`planning/ta-anchor-refactor-review.md`, still DRAFT) found 2 MAJOR + 5 coder-executability + minor findings; **all folded into the plan** (commits `68bda1a1`/`192ae06b`) — the **queueing-model path is disabled via an explicit-error dispatch refusal** (never a silent sat-v2 fallback; DEFERRED §12) and the **Mechanism A/B copy-mechanism choice is resolved** (§0: variant (ii)). Plan doc now internally consistent and coder-executable. Not pushed; no PR. Status: `session/status/ta-anchor-refactor.md`. Plan: [`planning/ta-anchor-refactor-plan.md`](../planning/ta-anchor-refactor-plan.md). | `a2f49ccf` (no commits) |
...
- **anchor-refactor mission (ta-anchor-goldens #1513 → ta-anchor-refactor PR-1 → ta-anchor-dynamic-refresh PR-2):**
  ...
  `a2f49ccf` (interim base; rebases onto `main` after #1513 merges); **not started (no code yet)**. The
  coder's 2 PR-1 questions are **resolved** (Dean, 2026-08-03; plan commit `500f08d8`): (1) **opt-in**
  enablement — a customized non-empty `analyzers:` list is taken literally (`[TA]` ⇒ TA-only), no
  `ApplyDefaults` change; (2) reuse the precomputed saturation result in the ballot-build (the load-bearing
  sat name-skip stays as a reuse guard, not deleted). A coding-plan review
  (`planning/ta-anchor-refactor-review.md`, still DRAFT) is **folded into the plan** (2026-08-04, commits
  `68bda1a1`/`192ae06b`): the queueing-model path is disabled via an explicit-error dispatch refusal
  (DEFERRED §12, not a silent fallback) and the **Mechanism A/B** copy-mechanism choice is resolved (§0).
  The `withSatEntry`-stability concern (goldens review Finding 2) is fixed in the same fold-in. Still
  open: mark the review FINAL (review agent/Dean's call); the design-doc §anchor "replace"-wording
  annotation (§12); and whether to file a GitHub issue for the queueing-model multi-analyzer-contract
  work (§12 DEFERRED bullet flags this — not blocking PR-1). PR-2 is a deferred stub (do NOT start until
  PR-1 lands). Plans: [`planning/ta-anchor-refactor-plan.md`](../planning/ta-anchor-refactor-plan.md),
  [`planning/ta-anchor-dynamic-refresh-plan.md`](../planning/ta-anchor-dynamic-refresh-plan.md).
