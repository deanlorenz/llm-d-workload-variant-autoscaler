...
- **TA3 (now):** #1250 at `f11f5120` — smoke PASS, maintainer ran `/ok-to-test` (Kind + OpenShift E2E running); await CI + ev-shindin re-review. **Leave origin/TA3 as-is — do NOT push.** Round-3 throughput fixes (F1–F4 + nits) planned as **local-only, droppable, one-per-fix commits, F1 first/standalone** (Dean selects which to push later). Plan: [`planning/TA3.1-plan.md`](../planning/TA3.1-plan.md) § Review-driven fixes (round 3); trigger `TA3__review-fixes-r3.md`. After merge: discuss E2E Step 2f; triage 3 pre-existing smoke failures (`smoke_test.go:339,:542,:1724`); ndots standalone PR.
