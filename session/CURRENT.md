...
- **TA3 (now):** #1250 at `f11f5120` — smoke PASS; **full E2E RED** (`throughput_analyzer_test.go:386` scale-up `[full]`): the round-2 opt-in gate prevents runtime TA registration, so the test that requires TA running times out. Required checks still green; full-E2E is `/ok-to-test`-triggered (ev-shindin ran it). **Leave origin/TA3 as-is — do NOT push.** Round-3 fixes (F1–F5 + nits) planned as **local-only, droppable, one-per-fix commits, F1 first/standalone**; **F5** = TA E2E tests Skip-when-unregistered + controller-restart-to-enable (the full-E2E green fix; companion to the shipped gate, not droppable if full-E2E matters). Plan: [`planning/TA3.1-plan.md`](../planning/TA3.1-plan.md) § Review-driven fixes (round 3); trigger `TA3__review-fixes-r3.md`. After merge: discuss E2E Step 2f; triage 3 pre-existing smoke failures (`smoke_test.go:339,:542,:1724`); ndots standalone PR.
