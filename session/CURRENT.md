...
| ta-veto-liveness | [#1481](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1481) | **OPEN, force-pushed 2026-07-29** (base upstream `main@dfc21e2c`; ev-shindin) — 8 commits, tip `c32235be`, GitHub **MERGEABLE**. Round-3 review **APPROVE, no outstanding findings**; round-2/3 folds landed: **D.1** reason-sentinel de-dup (`61060530`, exported `pipeline.ReasonNoData`/`ReasonError`), **D.2** `lastGoodAnalysis` departed-model prune, **D.3** warn-only demand-liveness detector (never sets `Live`/gates a decision) — D.2/D.3 in `c32235be` (amended in place to fold 3 round-3 NTH). All gates green incl. `-race`; DCO 8/8. **T1b closed** (reason-based `no-data`→non-live ruled correct + load-bearing). One accepted residual (F-B1 test builds its own `Live:true` result → won't guard a future deletion of that line). Awaiting ev-shindin re-review. Review: [`planning/ta-veto-liveness-review.md`](../planning/ta-veto-liveness-review.md) (Status: DRAFT until Dean finalizes). Plan: [`planning/ta-veto-liveness-plan.md`](../planning/ta-veto-liveness-plan.md). | `c32235be` |
| ta-model-level-demand | [#1480](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1480) | **OPEN, force-pushed 2026-07-29** (base upstream `main@dfc21e2c`; ev-shindin) — 6 commits, tip `b2acffd6`, GitHub **MERGEABLE**. Round-4 review **ready-to-push** (F1/F2/NTH-1 all closed). Round-2 = C.0 rebase + C.1/C.2 comment-only (zero-arrival safety; why RequestRate isn't an arrival cross-check); round-3/4 = **F3 §4a cleanup** (`b2acffd6`, 14 comment/test-desc sites, no logic change). All gates green; DCO 6/6. Known pre-existing upstream §4a leak left out of scope (dev-guide:671 + `analyzer_test.go` `F1`/`Specs 1–5 from plan §3.4` — from #1250; → separate cleanup, see Issues to Open). Awaiting ev-shindin re-review. Review: [`planning/ta-model-level-demand-review.md`](../planning/ta-model-level-demand-review.md). Plan: [`planning/ta-model-level-demand-plan.md`](../planning/ta-model-level-demand-plan.md). | `b2acffd6` |
| ta-gate-observability (PR E) | — (#1497) | **PLAN READY** — P0, I-5 half 2 (runtime ConfigMap-edit blind spot). Stopgap: K8s Warning event (reason `ThroughputAnalyzerRestartRequired`) + restart log when live config's TA-enabled state diverges from the frozen startup registration. Off `main` (moving-ref), independent of C/D. **No worktree/branch/PR yet** — awaiting Dean's go-ahead to create worktree + kickoff. Plan (TOC-refreshed, anchors re-verified vs `dfc21e2c` 2026-07-29): [`planning/ta-gate-observability-plan.md`](../planning/ta-gate-observability-plan.md). Tracker #1497; cross-repo doc companion #1498. | — |
| ta-correctness-guards (PR F) | — | **PLANNED — plan doc not yet authored** (hold until C merges, or pre-draft function-anchored against C's branch). P1 minimal guards: **I-6** wire real per-replica freshness from `trackMetricFreshness` into the emitted struct (currently hardcoded `Age:0/"fresh"` → dead staleness gate never fires) [LIVE]; **I-4** NaN/k*>1 guards in `computeLocalDemand` (unfiltered per `TODO(#1261)`) [LIVE gap]; **I-2** shared `validITLModel(A,B)` for `FitITLModel`+`resolveITLModel` (Tier-2 weaker) [minor]; **I-3** NaN-k* guard in `ObservationWindow.Add` [defense-in-depth]. Re-verify paths vs merged-C + #1450 rename. | — |
...
- **TA 0.9 coding — A + A′ MERGED; C + D force-pushed 2026-07-29, MERGEABLE, awaiting re-review.**
  Four Type 3 plans, all internally reviewed. **A [#1478](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1478)
  ...
  (tip `b2acffd6`, 6 commits) and **D [#1481](https://github.com/llm-d/llm-d-workload-variant-autoscaler/pull/1481)**
  (tip `c32235be`, 8 commits) were rebased onto the current `upstream/main` tip `dfc21e2c`, all
  round-2/3/4 review folds landed (C: F3 §4a cleanup; D: D.1/D.2/D.3), gates green, and
  **force-pushed to origin 2026-07-29 (Dean-authorized, `--force-with-lease`)**; GitHub confirms
  both **MERGEABLE**, OPEN. **Remaining follow-up:** (1) watch for ev-shindin review feedback on
  C + D; when they merge, flip PR Status rows to MERGED and close trackers #1495/#1496. (2)
  Reviewer replies drafted in `scratch/ta-cd-reviewer-replies-draft.md` — **held, unposted**;
  need Dean's per-comment approval + explicit `gh` before posting. No coder work outstanding
  otherwise. (D detail: per-tuple `(namespace, modelID, analyzerName)` keying confirmed correct by
  review; reason-based `no-data`→non-live liveness mechanism ruled correct + load-bearing; one
  accepted test residual.)
- **TA 0.9 remaining code work — PR E + PR F (see PR Status rows).** **PR E `ta-gate-observability`**
  (P0, I-5 half 2): plan authored + TOC-refreshed ([`planning/ta-gate-observability-plan.md`](../planning/ta-gate-observability-plan.md),
  tracker #1497). **Next action pending Dean's go-ahead:** create the `ta-gate-observability`
  worktree/branch off `main` (moving-ref) + kickoff trigger; new-branch push needs Dean's explicit
  confirmation. **PR F `ta-correctness-guards`** (P1, I-2/I-3/I-4/I-6): plan **not yet authored** —
  hold until C merges (or pre-draft function-anchored against C's branch). Re-derived scope: I-4 is
  a **LIVE gap** (`computeLocalDemand` runs on unfiltered metrics per `TODO(#1261)`, no NaN/k*>1
  guard); I-6 real (freshness hardcoded `Age:0/"fresh"` → dead staleness gate); I-2 minor (Tier-2
  `resolveITLModel` weaker than the already-hardened `FitITLModel`); I-3 defense-in-depth (`Add`
  is behind `report.OK()`). Reflect this honestly when authoring F (doc-accuracy discipline).
