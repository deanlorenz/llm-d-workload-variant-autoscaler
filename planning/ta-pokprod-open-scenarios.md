...
| 13 | **Commit the 83 uncommitted viz-refresh entries on the `benchmark` worktree** | ✅ **DONE 2026-08-17 — committed `bd9c375b`** (DCO signed, 103 files, +1100/−146). Verified before committing, not taken on trust: all 45 `panels.png` stamped `render_sha=a1a815a7` uniformly, all 16 `good-panels.png` staged as real symlinks (mode `120000`), nothing outside `runs/`. Branch now 35 ahead of origin, still unpushed. [[D-75]], [[D-78]] |
| 14 | **`reset_run.py`'s existence-check-not-completeness-check defect** | ✅ **FIXED 2026-08-17 — commit `02d43f89`** (DCO), +124 in `reset_run.py` + a new 439-line/19-test fixture suite. Now compares full file inventory **and** per-file byte size, refusing to delete on any missing file, size difference, or verification error. The corrected-report exception is **preserved as a surfaced mismatch**, not auto-resolved. Verified independently by the planner (19/19 tests run directly, source read, transcript audited — 4 shell commands, no cluster calls, no out-of-scope writes). First task completed by Bob as this scope's coder. [[D-74]], [[D-79]] |
