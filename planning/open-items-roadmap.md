...
| MA-F7 | `enabled:false` analyzer veto fix — `runAnalyzersAndScore` appends all registered analyzers unconditionally; disabled analyzer with Spare>0 vetoes scale-down. Fix: skip-the-run for disabled entries. **In PR-A** (`multi-analyzer-cleanup-plan.md`). | design.md §A8 / §F7 | post #1246 merge; unblocked | Q1 | S | — |
...
| ID | Title | Source | Gating | Q | Eff | Deps |
|---|---|---|---|---|---|---|
| MA-H-1 | Engine config-bridge tests — assert `runAnalyzersAndScore` correctly populates `Score`, `Disaggregated`, per-analyzer threshold overrides. **In PR-A** (`multi-analyzer-cleanup-plan.md` §Item 2). | optimizer-review.md Phase 4 | post #1246; unblocked | Q1 | S | — |
| MA-OPT-4 | Greedy fair-share with non-uniform `Score` — multi-model integration test, non-uniform Score across analyzers (T1.3 covers uniform case only). **In PR-A** (`multi-analyzer-cleanup-plan.md` §Item 2). | optimizer-review.md Phase 4 | post #1246; unblocked | Q1 | S | subset of `MA-H-1` |
...
| ID | Title | Source | Gating | Q | Eff | Deps |
|---|---|---|---|---|---|---|
| MA-OPT-1 | Multi-analyzer dev-guide expansion — stub exists on main (`docs/developer-guide/multi-analyzer-pipeline.md`); expand with user config, analyzer implementor guide, pipeline flow, veto semantics. **In PR-A** (`multi-analyzer-cleanup-plan.md` §Item 3). Note: ev-shindin #1223 adds `optimizers.md` (algorithms); PR-A doc covers pipeline flow — complementary. | optimizer-plan.md Open items | unblocked (#1246 merged) | Q1 | M | — |
