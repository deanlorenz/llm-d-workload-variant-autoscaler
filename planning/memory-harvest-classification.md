...
| FM14 | feedback_doc_names_not_numbers.md | `model` | This is really "how to talk about the taxonomy," directly the model doc's own subject matter (same class as C6/C11) rather than a per-step convention — no situational trigger fires it, it's a standing fact about vocabulary. Considered-and-rejected alternative: `conv:doc-ownership-boundary` or a new `conv:doc-taxonomy` — rejected because nothing situational triggers this fact the way a convention's `trigger:` field requires. |
