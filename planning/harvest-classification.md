...
| C3 | Repository layout (bare repo + worktrees tree diagram) | `model` | **Content moved 2026-08-18** to `plans-tooling/model/workspace-structure.md` § Repository Layout (`sec.sh`-fetchable), verbatim, byte-diffed clean. Descriptive of the workspace's physical structure, not a per-step rule — same tier as `conventions/`, addressed by `sec.sh` instead of `conv.sh`. |
...
| C6 | Document Taxonomy (Types 1–6, now design/epic-plan/code-spec/reference/review/session-state) | `model` | **Content moved 2026-08-18** to `plans-tooling/model/workspace-structure.md` § Document Taxonomy (`sec.sh`-fetchable), verbatim, byte-diffed clean. This *is* the taxonomy — explicitly the doc-and-session-model's own subject matter, already partially superseded by that doc. |
